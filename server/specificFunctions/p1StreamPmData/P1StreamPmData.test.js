...
describe("P1StreamPmData", () => {
  ...
  test("uses the MWDI source for processing when direct reads are enabled", async () => {
    loadRuntimeConfig.mockReturnValue({
      redis: {},
      service: {
        directMwdiReadEnabled: true,
        concurrency: 1,
        kafkaOutboundConcurrency: 1,
        kafkaConsumerTypes: "APT,ONF"
      }
    ...
    expect(startProcessingWorkerPoolRedis).toHaveBeenCalledWith(
      expect.objectContaining({
        mwdiReplicaEsClient: { uuid: "mwdiEsClient" }
      })
    );
  });

  test("does not initialize or start Kafka when Kafka is disabled", async () => {
    loadRuntimeConfig.mockReturnValue({
      redis: {},
      service: {
        kafkaEnabled: false,
    ...
    expect(p1InitKafka.run).not.toHaveBeenCalled();
    expect(startKafkaOutboundWorkerPool).not.toHaveBeenCalled();
    expect(startProcessingWorkerPoolRedis).toHaveBeenCalledWith(
      expect.objectContaining({ kafkaEnabled: false })
    );
    expect(result.kafkaConnectionList).toEqual([]);
  });
